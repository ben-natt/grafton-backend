...
async function exportBundleSheetToExcel(req, res) {
  try {
    ...
    if (!jobNo || !lotNo) {
      return res.status(400).send({ message: 'Job number and Lot number are required.' });
    }

    const data = await stockService.getIndividualBundleSheetData(jobNo, lotNo);
    const { mainDetails, bundles } = data;

    const workbook = new excel.Workbook();
    const worksheet = workbook.addWorksheet('Bundle Sheet', {
   ...
   // --- Style Definitions ---
    ...
    const headerData = [
        ['B3', 'Our Reference', mainDetails.ourReference],
        ['B4', 'Operator Job No', ''],
        ['B5', 'Operator Work Order', 'A'],
        ['G3', 'Commodity', mainDetails.commodity],
        ['G4', 'Shape', mainDetails.shape],
        ['G5', 'Brand', mainDetails.brand],
        ['L3', 'BL No', mainDetails.blNo],
        ['L4', 'Warehouse', mainDetails.warehouse],
        ['L5', 'Lot No /\nWarrant No', mainDetails.lotNoWarrantNo],
    ];
   ...
   // worksheet.getCell('M6').alignment = { wrapText: true };
    ...
    // Set specific background color for columns D and E headers
    ...
    // --- 5. Totals Row ---
    const totalRowIndex = dataStartRow + bundles.length;
   ...
   // applyBorderToRange(`Q1`, `R${currentRow-1}`, THICK_BLUE_BORDER);

    // --- Final Column Widths & File Sending ---
    worksheet.columns = [
        { width: 10 }, 
        { key: 'A', width: 20 },
    ...
    const fileName = `${jobNo}_Individual_Bundle_Sheet.xlsx`;
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    ...
    res.status(500).send({ message: 'Error exporting bundle sheet to Excel' });
  }
}
