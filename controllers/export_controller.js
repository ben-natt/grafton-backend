...
async function exportStocksToExcel(req, res) {
  try {
    ...
    if (stocks.length > 0 && stocks[0].ReleaseDate) {
      console.log(
        "Clean data received from database for ReleaseDate:",
        stocks[0].ReleaseDate
      );
    }
    ...
    const actionColIndex =
      worksheet.columns.findIndex((col) => col.header === "Action") + 1;
    if (actionColIndex > 0) {
      const actionHeaderCell = headerRow.getCell(actionColIndex);
      actionHeaderCell.font = {
        bold: true,
        size: 11,
        color: { argb: "FFFF0000" },
      };
    ...
    const processedData = stocks.map((stock, index) => {
      let weightValue = "-";
      if (stock.actualWeight && parseFloat(stock.actualWeight) !== 0) {
        weightValue = stock.actualWeight;
      }

      const displayLotNo = stock.LotNo;

      return {
        sno: index + 1,
        ...stock,
        NetWeight: weightValue, // Set NW to actualWeight or '-'
        GrossWeight: weightValue, // Set GW to actualWeight or '-'
        gwsLotNo: `${stock.JobNo} - ${displayLotNo}`,
      };
    });
...
async function exportBundleSheetToExcel(req, res) {
  try {
    ...
    if (!exWarehouseLots || exWarehouseLots.length === 0) {
      return res
        .status(404)
        .send({ message: "No lots found for the given job number to export." });
    }
    ...
    for (const exWarehouseLot of exWarehouseLots) {
      try {
        const data = await stockService.getIndividualBundleSheetData(
          jobNo,
          exWarehouseLot
        );
        ...
        const worksheetName =
          mainDetails && mainDetails.lotNoWarrantNo
        ...
        const headerData = [
          [
            "B3",
            "Our Reference",
            (mainDetails && mainDetails.ourReference) || "",
          ],
          ["B4", "Operator Job No", ""],
          ...
          [
            "G3",
            "Commodity",
            (mainDetails && mainDetails.commodity) || commodityName,
          ],
          ...
          [
            "L4",
            "Warehouse",
            (mainDetails && mainDetails.warehouse) || inboundWarehouseName,
          ],
          [
            "L5",
            "Lot No /\nWarrant No",
            mainDetails.lotNoWarrantNo || formatJobLot,
          ],
        ];
        ...
        headerData.forEach(([cell, label, value]) => {
          ...
          const valueCellAddress = cell.replace(/[A-Z]+/, (c) =>
          ...
          worksheet.mergeCells(
            `${valueCellAddress}:${String.fromCharCode(
              valueCellAddress.charCodeAt(0) + 1
            )}${valueCellAddress.slice(1)}`
          );

          // Apply border to merged cells
          const mergedEndCell = worksheet.getCell(
            `${String.fromCharCode(
              valueCellAddress.charCodeAt(0) + 1
            )}${valueCellAddress.slice(1)}`
          );
        ...
        worksheet.getCell("E7").value =
          "MISSING 100% WEIGHING DETAILS! DO NOW!";
        ...
        worksheet.getCell("M7").value =
          "If Yes, please indicate (Y) in the box";
        ...
        const headers = [
          "Bundle No",
          "Container No\nOR\nEx-Warrant No",
          "Producer Heat/Cast No",
          "Producer Bundle/Batch No",
          "Producer\nGW (KG)",
          "Producer\nNW (KG)",
          "100%\nGW (KG)",
          "Strapping\nWeight",
        ...
        tableHeaderRow.getCell("D").fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FFE2EFDA" },
        };
        tableHeaderRow.getCell("E").fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FFE2EFDA" },
        };
        tableHeaderRow.getCell("F").fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FFE2EFDA" },
        };

        tableHeaderRow.getCell("G").fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FFF2CC" },
        };
        tableHeaderRow.getCell("H").fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FFF2CC" },
        };
        tableHeaderRow.getCell("I").fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FFF2CC" },
        };
        ...
        totalRow.getCell("E").value = {
          formula: `SUM(E${dataStartRow}:E${totalRowIndex - 1})`,
          result: totalProducerGW,
        };
        totalRow.getCell("F").value = {
          formula: `SUM(F${dataStartRow}:F${totalRowIndex - 1})`,
          result: totalProducerNW,
        };
        totalRow.getCell("G").value = {
          formula: `SUM(G${dataStartRow}:G${totalRowIndex - 1})`,
          result: totalWeighedGW,
        };
        totalRow.getCell("I").value = {
          formula: `SUM(I${dataStartRow}:I${totalRowIndex - 1})`,
          result: totalWeighedNW,
        };
        ...
        worksheet.getCell(`H${diffRowIndex}`).fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FF000000" },
        };
        const totalProducerNWCell = worksheet.getCell(`I${diffRowIndex}`);
        totalProducerNWCell.value = {
          formula: `F${totalRowIndex} - I${totalRowIndex}`,
        };
        ...
        const listData = [
          ...
          {
            header: "COMMODITY",
            items: ["ALUMINIUM", "COPPER", "LEAD", "NICKEL", "TIN", "ZINC"],
          },
          {
            header: "SHAPE",
            items: [
        ...
        listData.forEach(({ header, items }) => {
          ...
          items.forEach((item, index) => {
            worksheet.mergeCells(
              `Q${itemStartRow + index}:R${itemStartRow + index}`
            );
            worksheet.getCell(`Q${itemStartRow + index}`).value = item;
            worksheet.getCell(`Q${itemStartRow + index}`).font = {
              name: "Arial",
              size: 9,
            };
          });
