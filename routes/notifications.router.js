...
router.get("/notifications/:status", async (req, res) => {
  try {
    ...
    combinedReports.sort(
      (a, b) => new Date(b.reportedOn) - new Date(a.reportedOn)
    );
    res.status(200).json({
...
router.delete("/notifications/:status/:reportId", async (req, res) => {
  try {
    ...
    if (success) {
      res.status(200).json({
        message: `Discrepancy report ${reportId} deleted successfully.`,
      });
    } else {
      res
        .status(404)
        .json({ error: `Discrepancy report with ID ${reportId} not found.` });
    }
  } catch (error) {
    console.error(
      `Error deleting discrepancy report ${req.params.reportId}:`,
      error
    );
    res.status(500).json({ error: "Internal server error" });
  }
 ...
 */
router.delete(
  "/notifications/duplicates/:status/:duplicatedId",
  async (req, res) => {
    try {
      ...
      if (success) {
        res.status(200).json({
          message: `Duplicate report ${duplicatedId} deleted successfully.`,
        });
      } else {
        res.status(404).json({
          error: `Duplicate report with ID ${duplicatedId} not found.`,
        });
      }
    } catch (error) {
      console.error(
        `Error deleting duplicate report ${req.params.duplicatedId}:`,
        error
      );
      res.status(500).json({ error: "Internal server error" });
    }
